...
function vectorSize(someVector) {
    ...
    return bufferutils.varIntSize(length) + someVector.reduce((sum, witness) => {
...
Transaction.fromBuffer = function (buffer, network = networks.default, __noStrict) {
    ...
    function readInt64() {
        const i = bufferutils.readInt64LE(buffer, offset);
        offset += 8;
        return i;
    }
    ...
    function readVarInt() {
        const result = bufferutils.readVarInt(buffer, offset);
        offset += result.size;
    ...
    function readJoinSplit() {
        ...
        const nullifiers = [];
        for (let j = 0; j < Transaction.ZCASH_NUM_JOINSPLITS_INPUTS; j++) {
    ...
    function readShieldedSpend() {
        const cv = readSlice(32);
        const anchor = readSlice(32);
        const nullifier = readSlice(32);
        const rk = readSlice(32);
        const zkproof = readZKProof();
        const spendAuthSig = readSlice(64);
        return {
            cv: cv,
            anchor: anchor,
            nullifier: nullifier,
    ...
    function readShieldedOutput() {
        const cv = readSlice(32);
        const cmu = readSlice(32);
        const ephemeralKey = readSlice(32);
        const encCiphertext = readSlice(580);
        const outCiphertext = readSlice(80);
        const zkproof = readZKProof();
    ...
    if (coins.isZcash(network)) {
        ...
        if (tx.supportsJoinSplits()) {
            const joinSplitsLen = readVarInt();
...
Transaction.prototype.getShieldedSpendByteLength = function () {
    ...
    let byteLength = 0;
    byteLength += bufferutils.varIntSize(this.vShieldedSpend.length);  // nShieldedSpend
...
Transaction.prototype.getShieldedOutputByteLength = function () {
    ...
    let byteLength = 0;
    byteLength += bufferutils.varIntSize(this.vShieldedOutput.length);  // nShieldedOutput
...
Transaction.prototype.getJoinSplitByteLength = function () {
    ...
    const joinSplitsLen = this.joinsplits.length;
    let byteLength = 0;
    ...
    if (joinSplitsLen > 0) {
        ...
        // 8 vpub_old, 8 vpub_new, 32 anchor, joinSplitsLen * 32 nullifiers, joinSplitsLen * 32 commitments,
...
Transaction.prototype.zcashTransactionByteLength = function () {
    ...
    byteLength += bufferutils.varIntSize(this.ins.length);  // tx_in_count
    byteLength += this.ins.reduce((sum, input) => {
        return sum + 40 + varSliceSize(input.script);
    }, 0);  // tx_in
    byteLength += bufferutils.varIntSize(this.outs.length);  // tx_out_count
...
Transaction.prototype.clone = function () {
    ...
    if (this.isSaplingCompatible()) {
        newTx.vShieldedSpend = this.vShieldedSpend.map((shieldedSpend) => {
            return {
                cv: shieldedSpend.cv,
        ...
        newTx.vShieldedOutput = this.vShieldedOutput.map((shieldedOutput) => {
            return {
    ...
    if (this.supportsJoinSplits()) {
        newTx.joinsplits = this.joinsplits.map((txJoinsplit) => {
            return {
...
Transaction.prototype.getBlake2bHash = function (bufferToHash, personalization) {
    // Use Node.js native BLAKE2b support
    const hash = crypto.createHash('blake2b512');
    hash.update(Buffer.from(personalization));
    hash.update(bufferToHash);
    return hash.digest().slice(0, 32); // Return first 32 bytes for 256-bit hash
};
...
Transaction.prototype.getPrevoutHash = function (hashType) {
    if (!(hashType & Transaction.SIGHASH_ANYONECANPAY)) {
        const bufferWriter = new BufferWriter(36 * this.ins.length);
...
Transaction.prototype.getSequenceHash = function (hashType) {
    if (!(hashType & Transaction.SIGHASH_ANYONECANPAY) &&
        ...
        const bufferWriter = new BufferWriter(4 * this.ins.length);
...
Transaction.prototype.hashForZcashSignature = function (inIndex, prevOutScript, value, hashType) {
    ...
    if (this.isOverwinterCompatible()) {
        ...
        const bufferWriter = new BufferWriter(baseBufferSize);
...
Transaction.prototype.hashForWitnessV0 = function (inIndex, prevOutScript, value, hashType) {
    ...
    const bufferWriter = new BufferWriter(156 + varSliceSize(prevOutScript));
    const input = this.ins[inIndex];
...
Transaction.prototype.__toBuffer = function (buffer, initialOffset, __allowWitness) {
    ...
    function writeVarInt(i) {
        bufferutils.varIntBuffer(i, buffer, offset);
        offset += bufferutils.varIntBuffer.bytes;
    }
    ...
    if (this.isSaplingCompatible()) {
        ...
        this.vShieldedSpend.forEach((shieldedSpend) => {
        ...
        this.vShieldedOutput.forEach((shieldedOutput) => {
