...
function generate(array, options, callback) {
    ...
    try {
        ...
        if (options.reverse === false) {
            ...
            const merkle = recursiveMerkle(reversedHashes);
            merkle.root = merkle.root.match(/.{2}/g).reverse().join('');
            if (callback) return callback(null, merkle);
            return merkle;
        }
    } catch (err) {
...
function recursiveMerkle(hashes) {
    ...
    for (let i = 0, length = hashes.length; i < length; i += 2) {
        concatHashes.push(hashes[i] + hashes[i + 1]);
    }

    /* Map every element of the new array with a hash function */
    const newHashes = concatHashes.map(data => doubleHash(data));
