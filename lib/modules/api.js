...
function processBlockRecord(obj, coinSymbol, forkId) {
    isWriting = true;
    ...
    try {
        ...
        let data;
        try {
            data = fs.readFileSync(blocksFilePath, 'utf8');
        } catch (err) {
            if (err.code === 'ENOENT') {
                // File doesn't exist, create it
                const array = [obj];
                fs.writeFileSync(blocksFilePath, JSON.stringify(array));
        ...
        object.push(obj);
        fs.writeFileSync(blocksFilePath, JSON.stringify(object));
        logging('Api', 'debug', 'Block ' + obj.block + ' recorded. Hash: ' + obj.hash.substring(0, 16) + '... Finder: ' + obj.finder, forkId);
