...
const BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, extraNoncePlaceholder, reward, recipients, poolAddress, coin, pubkey) {
    ...
    case (((((this.difficulty * Math.pow(2, 32)) / 60) / Math.pow(10, 15)).toFixed(2)) > 1):
        nethash = ((((this.difficulty * Math.pow(2, 32)) / 60) / Math.pow(10, 15)).toFixed(2));
        if (!process.env.forkId || process.env.forkId === '0') {
            logging('Blocks', 'warning', 'Effective nethash is: ' + nethash + ' Msol/s');
        }
        break;
    case (((((this.difficulty * Math.pow(2, 32)) / 60) / Math.pow(10, 12)).toFixed(2)) > 1):
        nethash = ((((this.difficulty * Math.pow(2, 32)) / 60) / Math.pow(10, 12)).toFixed(2));
        if (!process.env.forkId || process.env.forkId === '0') {
            logging('Blocks', 'warning', 'Effective nethash is: ' + nethash + ' ksol/s');
        }
        break;
    case (((((this.difficulty * Math.pow(2, 32)) / 60) / Math.pow(10, 9)).toFixed(2)) > 1):
        nethash = ((((this.difficulty * Math.pow(2, 32)) / 60) / Math.pow(10, 9)).toFixed(2));
        if (!process.env.forkId || process.env.forkId === '0') {
            logging('Blocks', 'warning', 'Effective nethash is: ' + nethash + ' sol/s');
        }
        break;
    }
    // Recalculate difficulty for logging (same calculation as above)
    this.difficulty = parseFloat(Number(difficultyBigInt).toFixed(9));
    if (!process.env.forkId || process.env.forkId === '0') {
