...
const pool = module.exports = function pool(options, authorizeFn) {
    ...
    const emitLog = function (text) {
        logging('Stratum', 'debug', text);
    };
    const emitWarningLog = function (text) {
        logging('Stratum', 'warning', text);
    };
    const emitErrorLog = function (text) {
        logging('Stratum', 'error', text);
    };
    const emitSpecialLog = function (text) {
        logging('Stratum', 'special', text);
    };
    ...
    // Listen for blocknotify messages from master process
    process.on('message', function (msg) {
        if (msg.type === 'blocknotify') {
            _this.processBlockNotify(msg.hash, 'rpc');
        }
    });
    ...
    function OutputPoolInfo() {
        const startMessage = 'Stratum Pool Server Started for ' + options.coin.name + ' [' + options.coin.symbol.toUpperCase() + ']';
    ...
    function SubmitBlock(blockHex, callback) {
        const rpcCommand = 'submitblock';
        const rpcArgs = [blockHex];
            ...
            _this.daemon.cmd('decoderawtransaction',
                ...
                function (result) {
                    if (result.error) {
                        ...
                        const processedNewBlock = _this.jobManager.processTemplate(template);
