...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    //TODO(alstr): fix init print math
    function OutputPoolInfo() {
        ...
        var vnethash = ((((_this.jobManager.currentJob.difficulty * Math.pow(2, 32)) / 60) / Math.pow(10, 15))*8.192).toFixed(2)
        var infoLines = [startMessage,
            'Miner Connectable Port(s):\t' + _this.options.initStats.stratumPorts.join(', '),
            'Pool Thread Count:\t\t' + (options.clustering.enabled ? options.clustering.forks : '1'),
            'Network Connection Type:\t' + (options.testnet ? 'Testnet' : 'Mainnet'),
            'Network Connected Peers:\t' + options.initStats.connections,
            'Daemon Math...',
            ...
            'Block Template Math...',
            'Current Hash Rate Needed:\t' + vnethash + 'M Sol/s',
    ...
    function SubmitBlock(blockHex, callback) {
        ...
        function (results) {
            ...
            if (options.printSubmissions) {
                logging('Blocks', 'debug', 'Submitted Block using ' + rpcCommand + ' successfully to daemon instance(s)');
            }
            callback();
        }
                        );
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            client.on('difficultyChanged', function (diff) {
                ...
                We decide if:
                a) we want to use a literal value we give it in config (true)
                or
                ...
                */
                ...
                //console.log('client.difficulty: '+client.difficulty);
    ...
    this.relinquishMiners = function (filterFn, resultCback) {
        ...
        async.filter(stratumClients, filterFn, function (clientsToRelinquish) {
            ...
            process.nextTick(function () {
                resultCback(clientsToRelinquish.map(function (item) { return item.client; }));
            });
        })
    };
