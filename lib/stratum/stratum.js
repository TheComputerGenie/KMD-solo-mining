...
const StratumClient = function (options) {
    ...
    function handleSubscribe(message) {
        ...
        _this.emit('subscription',
            {},
            // second parameter name was duplicated; keep API (two identical values) but clarify naming
            (error, extraNonce1, extraNonce1Dup) => {
                if (error) {
                    sendJson({
                        id: message.id,
                        result: null,
    ...
    function handleAuthorize(message) {
        ...
        options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, _this.workerPass, (result) => {
    ...
    function handleSubmit(message) {
        ...
        _this.emit('submit',
            ...
            (error, result) => {
                ...
                _this.emit('share', error, result);

            }
        );
    }

    // Faster concatenation using Array.join; reduces intermediate string churn
    function sendJson() {
        if (arguments.length === 1) {
            // common hot path
            options.socket.write(`${JSON.stringify(arguments[0])}\n`);
            return;
        }
        const arr = new Array(arguments.length);
        for (let i = 0; i < arguments.length; i++) {
            arr[i] = JSON.stringify(arguments[i]);
        }
        options.socket.write(`${arr.join('\n')}\n`);
    }
    ...
    this.sendDifficulty = function (difficulty) {
        ...
        if (!options.algorithm || typeof options.algorithm.getDiff1 !== 'function') {
            if (!_this._algoWarned) {
                _this._algoWarned = true;
                _this.emit('socketError', new Error('Algorithm missing for difficulty calculation; skipping target set'));
            }
            return false; // Do not attempt target update
        }
        const powLimitBig = options.algorithm.getDiff1();
        const adjPowBig = difficulty > 0 ? (powLimitBig / BigInt(Math.floor(difficulty))) : 0n;
        // Convert back to number-based string formatting similar to previous logic
        const adjPowHex = adjPowBig.toString(16);
        let zeroPad;
        if ((64 - adjPowHex.length) === 0) {
            zeroPad = '';
        } else {
            zeroPad = '0'.repeat(64 - adjPowHex.length);
        }
        const target = (zeroPad + adjPowHex).substr(0, 64);
...
const StratumServer = exports.Server = function StratumServer(options, authorizeFn) {
    ...
    this.broadcastMiningJobs = function (jobParams) {
        for (const clientId in stratumClients) {
            if (!Object.prototype.hasOwnProperty.call(stratumClients, clientId)) {
                continue;
            } // safeguard
