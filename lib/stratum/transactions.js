...
// TODO: Clean up
// Since this is a hodgepodge of years of crammed in crap, it needs sorted.
// labels: enhancement
...
exports.createGeneration = function( blockHeight, blockReward, feeReward, recipients, poolAddress, coin, pubkey, vouts) {
  ...
  for (var i = 0; i < lengthDiff; i++) { blockHeightSerial = blockHeightSerial + "00"; }
  length = "0" + height;
  ...
  for (i = 0; i <= vouts.length - 1; i++) {
    ...
    if (i == 0) {
      if (coin.toUpperCase() === "KMD") {
        if (!process.env.forkId || process.env.forkId === "0") { logging("Blocks",  amt > 1 * Math.pow(10, 9) ? "error" : "debug", blockHeight + " would pay: " + amt / Math.pow(10, 8)); }
      } else {
        if (!process.env.forkId || process.env.forkId === "0") { logging("Blocks", "debug", blockHeight + " would pay: " + amt / Math.pow(10, 8)); }
      }
    }

// TODO: Clean up script types
// Since this is a hodgepodge of years of crammed in crap, it needs sorted.
// labels: enhancement
