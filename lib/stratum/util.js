...
const getReadableHashRateString = (rate, algo) => {
    // Delegate to algorithm-specific formatter if available
    if (algo && typeof algo.formatHashRate === 'function') {
        return algo.formatHashRate(rate);
    }
    // Generic fallback (hashes per second)
    let r = rate;
    let i = -1;
