...
// TODO: examine and see how much is antiquated leftovers incuding is vardiff even working?
/*
    ...
    this.manageClient = function (client) {
        ...
        client.on('submit', function () {
            ...
            if (avg > tMax && client.difficulty > options.minDiff) {
                if (options.x2mode) { ddiff = 0.5; }
                if (ddiff * client.difficulty < options.minDiff) { ddiff = options.minDiff / client.difficulty; }
            } else if (avg < tMin) {
                if (options.x2mode) { ddiff = 2; }
                var diffMax = options.maxDiff;
                if (ddiff * client.difficulty > diffMax) { ddiff = diffMax / client.difficulty; }
            }
            else {
                return;
            }
