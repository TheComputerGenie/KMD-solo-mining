...
function varIntEncode(number, buffer, offset) {
  if (!buffer) buffer = Buffer.allocUnsafe(varIntEncodingLength(number))
  if (!offset) offset = 0
...
function pushDataEncode(data, buffer, offset) {
  ...
  var encodingLength = pushDataEncodingLength(length)

  if (!buffer) buffer = Buffer.allocUnsafe(encodingLength + length)
  if (!offset) offset = 0
...
function writeUInt64LE(buffer, value, offset) {
  if (typeof value === 'number') value = global.BigInt(value)
...
function readVarInt(buffer, offset) {
  var result = varIntDecode(buffer, offset)
