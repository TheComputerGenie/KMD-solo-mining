...
function modInverse(a, m) {
    ...
    if (x < global.BigInt(0)) {
        x += m0
    }
    return x
}

const n = global.BigInt(secp256k1.n.toString())
const N_OVER_TWO = n >> global.BigInt(1)

function sign(hash, d) {
    typeforce(types.tuple(types.Hash256bit, types.BigInt), arguments)

    const x = bigIntToBuffer(d, 32)
    const e = bigIntFromBuffer(hash)
    const G = secp256k1.G

    let r, s
    deterministicGenerateK(hash, x, function (k) {
        const Q = G.multiply(BigInteger(k.toString()))
        ...
        if (r === global.BigInt(0)) {
            return false
        }

        s = (modInverse(k, n) * (e + d * r)) % n
        if (s === global.BigInt(0)) {
            return false
        }
...
function verify(hash, signature, Q) {
    ...
    if (r <= global.BigInt(0) || r >= n) {
        return false
    }
    if (s <= global.BigInt(0) || s >= n) {
        return false
    }
    ...
    //               R = u1G + u2Q
    const R = G.multiplyTwo(BigInteger(u1.toString()), Q, BigInteger(u2.toString()))
    ...
    const v = global.BigInt(xR.toString()) % n
