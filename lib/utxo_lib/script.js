...
function decompile(buffer) {
  ...
  while (i < buffer.length) {
    ...
    if ((opcode > OPS.OP_0) && (opcode <= OPS.OP_PUSHDATA4)) {
      var data = bufferutils.readPushDataInt(buffer, i)
      ...
      if (i + data.length > buffer.length) return []
...
function isDERSignature(buffer) {
  ...
  var length = buffer[1]
  if (length !== buffer.length - 2) return false

  var rOffset = 2
  if (buffer[rOffset] !== 0x02) return false

  var rLength = buffer[rOffset + 1]
  if (rLength === 0) return false
  if (rOffset + rLength + 2 >= buffer.length) return false
  if (buffer[rOffset + 2] & 0x80) return false
  if (rLength > 1 && buffer[rOffset + 2] === 0x00 && !(buffer[rOffset + 3] & 0x80)) return false

  var sOffset = rOffset + rLength + 2
  if (buffer[sOffset] !== 0x02) return false

  var sLength = buffer[sOffset + 1]
  if (sLength === 0) return false
  if (sOffset + sLength + 2 !== buffer.length) return false
  if (buffer[sOffset + 2] & 0x80) return false
  if (sLength > 1 && buffer[sOffset + 2] === 0x00 && !(buffer[sOffset + 3] & 0x80)) return false
