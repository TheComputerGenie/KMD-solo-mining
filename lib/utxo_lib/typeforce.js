...
function typeforce(validator, value, allowIncomplete) {
    if (typeof validator === 'function') {
        ...
        for (var key in validator) {
            if (allowIncomplete && value[key] === undefined) continue
        ...
        for (var i = 0; i < validator.length && i < value.length; i++) {
            if (allowIncomplete && value[i] === undefined) continue
...
typeforce.tuple = function () {
    ...
    return function (values) {
        ...
        if (values.length !== validators.length) return false
...
module.exports.tuple = typeforce.tuple